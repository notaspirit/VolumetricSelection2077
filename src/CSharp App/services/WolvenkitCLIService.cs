    ...
    public class WolvenkitCLIService
    {
        ...
        public async Task<(bool success, string error, Dictionary<string, byte[]>? files)> ExtractBulkJsonFiles(List<string> filePaths)
        {
            ...
            foreach (var archivePath in archivePaths)
            {
                ...
                if (!string.IsNullOrEmpty(extractError))
                {
                    Logger.Warning($"Failed to extract files from archive {archivePath}: {extractError}");
                    continue;
                }
            }
            ...
            try
            {
                foreach (var filePath in filePaths)
                {
                    ...
                    // Check if files exist and add them to tracking list
                    bool hasJson = File.Exists(filePathAbsJson);
                    bool hasRaw = File.Exists(filePathAbsRaw);
                    ...
                    if (hasJson)
                    {
                        ...
                        {
                            Logger.Warning($"Failed to read JSON file {filePath}: {ex.Message}");
                            continue;
                        }
                    }
                    ...
                    if (!hasJson || !hasRaw)
                    {
                        Logger.Warning($"Missing extracted file(s) for {filePath}");
                        continue;
                    }
                }
                
                // Return success if we got any files
                return (files.Count > 0, string.Empty, files);
            }
            catch (Exception ex)
            {
                Logger.Warning($"Failed to read extracted files: {ex.Message}");
