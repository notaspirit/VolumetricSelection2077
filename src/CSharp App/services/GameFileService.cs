...
namespace VolumetricSelection2077.Services
{
    public class GameFileService
    {
        ...
        public async Task<(bool success, string error, List<byte[]>? files)> GetBulkMPackFiles(List<string> filePaths)
        {
            ...
            */
            Logger.Info($"Getting {filePaths.Count} files");
            string[] cleanFilePaths = filePaths.Select(f => new string(f.Where(c => !char.IsControl(c)).ToArray())).ToArray();
            var firstCacheResults = _cacheService.GetEntries(CacheDatabase.ExtractedFiles.ToString(), cleanFilePaths);
            ...
            Logger.Info($"Found {missingFiles.Count} missing files");
            if (missingFiles.Count > 0)
            {
                ...
                var preprocessedFiles = _preprocessingService.PreprocessStreamingSector(files);
