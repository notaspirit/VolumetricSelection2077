...
function createSelectionBox() {
    const {minPoint, maxPoint} = validateInputBoxMinMax(InputJson.selectionBox.min, InputJson.selectionBox.max);
    return new Cube(minPoint, maxPoint, InputJson.selectionBox.quat);
}
...
function doCubeIntersectWithSelectionBox(cube) {
...
// Simplified version of doCubeIntersectWithSelectionBox for testing
function doCubeIntersectWithSelectionBox(cube) {
...
// TODO: Remove the Logger.Info statements, add actor support
// Function to create AXL deletion file
function createAXLDeletionFile(json) {
    let deletionString = "streaming:\n  sectors:\n";
//    Logger.Info("Processing JSON: " + JSON.stringify(json));

    for (let item of json) {
        if (item.sector) {
            let sector = item.sector;
//            Logger.Info("Processing sector: " + JSON.stringify(sector));
            if (sector.nodes && Array.isArray(sector.nodes) && sector.nodes.length > 0) {
                deletionString += `    - path: ${sector.name}\n      expectedNodes: ${sector.expectedNodes}\n      nodeDeletions:\n`;
//                Logger.Info("Sector nodes: " + JSON.stringify(sector.nodes));
                for (let node of sector.nodes) {
//                    Logger.Info("Processing node: " + JSON.stringify(node));
                    deletionString += `        - index: ${node.Index}\n          type: ${node.Type}\n`;
                }
            } else {
//                Logger.Success("Sector has no nodes or nodes is not an array: " + sector.name);
            }
        } else {
 //           Logger.Success("Item in JSON array does not have a 'sector' property");
        }
    }

    wkit.SaveToResources("CollisionDetection.xl", deletionString);
    Logger.Info("CollisionDetection.xl saved to resources!");
    // Logger.Info("Final deletion string: " + deletionString);
}

// Function to build cube from point and scale
function buildCubeFromPointAndScale(point, scale) {
    let boxMin = point;
    let boxMax = {x: point.x + scale.x, y: point.y + scale.y, z: point.z + scale.z};
    return new Cube(boxMin, boxMax, {i: 0, j: 0, k: 0, r: 1});
}
    ...
    for (let nodeDataIndex in nodeData) {
        for (let nodeInfoIndex in NodeInfo) {
            if (nodeData[nodeDataIndex]["NodeIndex"] === NodeInfo[nodeInfoIndex]["index"]) {
                Logger.Info(nodeData[nodeDataIndex]);
                // Build Cube
                let UcubeMin = {x: nodeData[nodeDataIndex]["Bounds"]["Min"]["X"], y: nodeData[nodeDataIndex]["Bounds"]["Min"]["Y"], z: nodeData[nodeDataIndex]["Bounds"]["Min"]["Z"], w: nodeData[nodeDataIndex]["Bounds"]["Min"]["W"]};
                let UcubeMax = {x: nodeData[nodeDataIndex]["Bounds"]["Max"]["X"], y: nodeData[nodeDataIndex]["Bounds"]["Max"]["Y"], z: nodeData[nodeDataIndex]["Bounds"]["Max"]["Z"], w: nodeData[nodeDataIndex]["Bounds"]["Max"]["W"]};
                ...
                if (UcubeMin.x == UcubeMax.x && UcubeMin.y == UcubeMax.y && UcubeMin.z == UcubeMax.z && UcubeMin.z == UcubeMax.z) {
                    pointCount += 1;
                    
                    let { minPoint, maxPoint } = validateInputBoxMinMax(UcubeMin, UcubeMax);
                    let scale = {x: nodeData[nodeDataIndex]["Scale"]["X"], y: nodeData[nodeDataIndex]["Scale"]["Y"], z: nodeData[nodeDataIndex]["Scale"]["Z"]};
                    let cube = buildCubeFromPointAndScale(minPoint, scale);
                    if (doCubeIntersectWithSelectionBox(cube) === true) {
                        nodesOutput.push({"Index":Number(nodeDataIndex),"Type":NodeInfo[Number(nodeInfoIndex)]["type"]});
                    }
                } else {
                    cubeCount += 1;
                    cubeInfo.push({"Index":nodeDataIndex,"Type":NodeInfo[nodeInfoIndex]["type"],"Min":UcubeMin,"Max":UcubeMax,"Quat":cubeQuat});
                    let { minPoint, maxPoint } = validateInputBoxMinMax(UcubeMin, UcubeMax);
                    if (doCubeIntersectWithSelectionBox(new Cube(minPoint, maxPoint, cubeQuat)) === true) {
                        nodesOutput.push({"Index":Number(nodeDataIndex),"Type":NodeInfo[Number(nodeInfoIndex)]["type"]});
                    }
                }

/* For Testing Remove Actor Checking
                    ...
                    if (NodeInfo[nodeInfoIndex]["actors"] !== null) {
                        let actorOutput = [];
                        let actorIndex = 0;
                        Logger.Info(NodeInfo[nodeInfoIndex]["actors"]);
                        Logger.Info(NodeInfo[nodeInfoIndex]);
                        for (let actor of NodeInfo[nodeInfoIndex]["actors"]) {
                            ...
                            if (actor["Shapes"]["ShapeType"] === "Box") {
                                ...
                                Logger.Info(absoluteActorCubeMin);
                                Logger.Info(absoluteActorCubeMax);  
                            ...
                            } else if (actor["Shapes"]["ShapeType"] === "Sphere") {
...
*/
    ...
    OutputJson.push({"sector": {"name": sectorPath, "expectedNodes": nodeData.length, "nodes": nodesOutput}});
}
// Logger.Info(OutputJson);
createAXLDeletionFile(OutputJson);
for (let item of OutputJson) {
    Logger.Info(item.sector.name);
    
}
Logger.Info("Point Count: " + pointCount);
Logger.Info("Cube Count: " + cubeCount);

Logger.Info(cubeInfo);
