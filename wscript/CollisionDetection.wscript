...
function applyQuaternionRotation(vertex, quat) {
    ...
    const ix = qw * vertex.x + qy * vertex.z - qz * vertex.y;
    const iy = qw * vertex.y + qz * vertex.x - qx * vertex.z;
    const iz = qw * vertex.z + qx * vertex.y - qy * vertex.x;
    const iw = -qx * vertex.x - qy * vertex.y - qz * vertex.z;
...
function isVertexInsideSelectionBox(vertex) {
    ...
    return (
        vertex.x >= minPoint.x && vertex.x <= maxPoint.x &&
        vertex.y >= minPoint.y && vertex.y <= maxPoint.y &&
        vertex.z >= minPoint.z && vertex.z <= maxPoint.z &&
        vertex.w >= minPoint.w && vertex.w <= maxPoint.w
    );
}
    ...
    for (let nodeDataIndex in nodeData) {
        for (let nodeInfoIndex in NodeInfo) {
            if (nodeData[nodeDataIndex]["NodeIndex"] === NodeInfo[nodeInfoIndex]["index"]) {
                ...
                let cubeQuat = {x: nodeData[nodeDataIndex]["Orientation"]["i"], y: nodeData[nodeDataIndex]["Orientation"]["j"], z: nodeData[nodeDataIndex]["Orientation"]["k"], w: nodeData[nodeDataIndex]["Orientation"]["r"]};
